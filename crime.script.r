...
if( !file.exists( "./crime_data" )){ 
    dir.create( "./crime_data" )
}                                         # Create "crime_data" directory
...
geo_1 <- read_csv( file = url_1, 
                   col_names = cols, 
                   col_types = types, 
                   trim_ws = TRUE )           # Read in first results

geo_2 <- read_csv( file = url_2,
                   col_names = cols, 
                   col_types = types, 
                   trim_ws = TRUE )           # Read in second results
...
crime$geoid <-  paste0( crime$state , 
                        crime$county , 
                        crime$tract )      # Paste padded FIPS codes: "geoid"

crime <- crime %>% 
    mutate( tract = str_sub( string = tract,
                             start = 3, 
                             end = 6 ),
...
crime$tract <- gsub( x = crime$tract, 
                     pattern = "^(.{2})(.{2})$", 
                     replacement = "\\1\\.\\2" )         # Insert "." in "tract"

crime <- crime %>%
    select( month, geoid, tract, crime )                 # Rearrange columns
...
crime <- crime %>%
    filter( month < date( "2018-06-01" ))     # Remove current month data (06-18)
